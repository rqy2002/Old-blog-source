...
#+begin_proof
a) 首先考虑到 $\dim X = \sup \{ n \mid V_0 \subsetneq V_1 \subsetneq \dots \subsetneq V_n \}$,
   其中 $V_k$ 都是 $X$ 中的不可约闭集. 
...
b) 在 a 中已证.
c) 由定义, $\codim(Y, X) = \inf_{V \text{不可约} \subseteq Y} \codim(V, X)$.
   取一般点即证.
d) 对任意点 $P$, 设其有仿射邻域 $\Spec A$, 在 $A$ 中对应素理想 $\gp$.
...
d)
  1. 由 b), $E_e = X$.
  2. 由 c), 若 $h > e$, 则 $E_h \subseteq X \setminus U$.
     因此 $E_h$ 不稠密.
  3. 暂且不会, 呜呜.
e) 由于 $E_h$ 闭, 由 3.19, $C_h = f(E_h) \setminus f(E_{h + 1})$ 可构造.
